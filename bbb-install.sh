...
# If we are using an IP and not a domain, generate a self-signed SSL cert
  if [[ "$HOST" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      ...
      openssl req -x509 -nodes -days 365 \
          ...
          -subj "/C=US/ST=NA/L=NA/O=BigBlueButton/OU=Dev/CN=$HOST"
  fi

  # Disable automatic Let's Encrypt SSL when using an IP address.
  if [ -n "$PROVIDED_CERTIFICATE" ] ; then
      install_ssl
  elif [[ "$HOST" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      echo "Skipping Let's Encrypt SSL for IP address setup."
  else
      install_ssl
  fi
...
check_host() {
  # If user provides an IP, skip DNS check
...
{ cat /etc/ssl/bigbluebutton/bigbluebutton.crt; echo; cat /etc/letsencrypt/live/$HOST/privkey.pem; } > /etc/haproxy/certbundle.pem.new
...
install_ssl() {
  ...
  if [ -z "$PROVIDED_CERTIFICATE" ]; then
    ...
    if [[ -f "/etc/ssl/bigbluebutton/bigbluebutton.crt" ]] && [[ -f "/etc/letsencrypt/renewal/$HOST.conf" ]] \
  ...
  if [ ! -f "/etc/ssl/bigbluebutton/bigbluebutton.crt" ]; then
...
HERE
    ...
    if [ -z "$PROVIDED_CERTIFICATE" ]; then
      ...
      ln -s /local/certs/fullchain.pem "/etc/ssl/bigbluebutton/bigbluebutton.crt"
      ln -s /local/certs/privkey.pem "/etc/ssl/bigbluebutton/bigbluebutton.key"
    fi
  fi
...
server {
  ...
  server_name $HOST;
    ...
    ssl_certificate /etc/ssl/bigbluebutton/bigbluebutton.crt;
    ssl_certificate_key /etc/ssl/bigbluebutton/bigbluebutton.key;
